...
export default function AdminPage() {
  ...
  const handleUpload = async (e: any) => {
    ...
    reader.onload = async (evt) => {
      ...
      rows.slice(1).forEach((row, i) => {
        ...
        const qtd = parseInt(row[colToIdx(rule.colQuantidade)], 10) || 1;
        const liquidoTotal = repasse * qtd;
        const estadoUF = (row[colToIdx(rule.colEstado)] || 'SP').toString().substring(0,2).toUpperCase();

        // 1. Objeto no padrão exato da tabela Supabase
        novosParaBanco.push({
          id_pedido: row[colToIdx(rule.colIdPedido)] || `PED-${Date.now()}-${i}`,
          data_faturamento: new Date().toISOString().slice(0, 10),
          canal: selectedChannel,
          estado_compra: estadoUF,
          sku: row[colToIdx(rule.colSku)] || 'SKU-GENERAL',
          faturamento_liquido_final: liquidoTotal
        });

        // 2. Objeto visual para o Frontend (inclui campos auxiliares para cálculo de KPI)
        novosParaTela.push({
          id_pedido: row[colToIdx(rule.colIdPedido)] || `PED-${Date.now()}-${i}`,
          data_faturamento: new Date().toISOString().slice(0, 10),
          canal: selectedChannel,
      ...
      // Grava no Supabase em Lote (Bulk Insert)
      if (novosParaBanco.length > 0) {
        const { error } = await supabase.from('tb_vendas').insert(novosParaBanco);
        
        if (error) {
          addLog(`Falha ao gravar no Supabase: ${error.message}`, 'error');
        } else {
          setSales((prev: any) => [...novosParaTela, ...prev]);
          addLog(`${novosParaBanco.length} vendas gravadas no Supabase com sucesso.`, 'success');
        }
      }
  ...
  if (!isAdminUnlocked) {
    return (
      <div className="max-w-md mx-auto bg-slate-900 p-8 rounded-2xl border border-slate-800 text-center mt-10">
        <h2 className="text-xl font-bold text-white mb-4">Área Restrita</h2>
        <form onSubmit={auth}><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Senha Master (Dash321)" className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl mb-4 text-white" /><button className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl">Desbloquear</button></form>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
       <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800">
         <h2 className="font-bold text-white mb-2">Importação de Planilha do Canal</h2>
         <p className="text-xs text-slate-400 mb-4">A importação gravará os registros diretamente na tabela tb_vendas do Supabase.</p>
         
         <select value={selectedChannel} onChange={e => setSelectedChannel(e.target.value)} className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl mb-4 text-purple-300 font-bold">
           {CHANNELS.map(ch => <option key={ch}>{ch}</option>)}
         </select>
         
         <label className={`cursor-pointer block py-3 text-white font-bold text-center rounded-xl transition ${isProcessing ? 'bg-slate-600' : 'bg-purple-600 hover:bg-purple-500'}`}>
           <input type="file" className="hidden" accept=".xlsx, .csv" onChange={handleUpload} disabled={isProcessing}/>
           {isProcessing ? 'A sincronizar com banco...' : `Importar e Gravar Vendas (${selectedChannel})`}
         </label>
       </div>

       <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800">
          <h3 className="font-bold text-white mb-2">Console de Integração (Supabase)</h3>
