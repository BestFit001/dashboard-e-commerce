...
export default function Home() {
  ...
  // Mapeamento ajustado para corresponder aos nomes reais das colunas da planilha do canal
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        throw new Error('O servidor retornou uma página de erro em vez de JSON.');
      }
  ...
  return (
    <main style={{ padding: '40px', fontFamily: 'Arial, sans-serif', background: '#0b0f19', color: '#fff', minHeight: '100vh' }}>
      ...
      <p style={{ color: '#888', marginBottom: '30px' }}>Painéis isolados por tipo de base e mapeamento dedicado por canal.</p>
      ...
      {/* 1. PAINEL DE VENDAS E CANAL */}
      <form onSubmit={handleSubmit} style={{ background: '#161b22', padding: '30px', borderRadius: '12px', border: '1px solid #30363d', marginBottom: '30px' }}>
        <h2 style={{ fontSize: '18px', color: '#2ea043', marginBottom: '15px' }}>1. Painel de Pedidos e Mapeamento do Canal</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', marginBottom: '20px' }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#aaa', marginBottom: '5px' }}>Coluna Preço de Venda:</label>
            <input 
              type="text" 
          ...
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#aaa', marginBottom: '5px' }}>Coluna Rebate (Bônus):</label>
            <input 
              type="text" 
        ...
        <button 
          type="submit" 
          disabled={carregando}
          style={{ background: '#238636', color: '#fff', padding: '12px 24px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
        >
          {carregando ? 'A processar...' : 'Processar Canal e Calcular Liquidez'}
        </button>
      </form>

      {/* 2. PAINEL DE BASE DE CUSTOS (SUPABASE) */}
      <div style={{ background: '#161b22', padding: '30px', borderRadius: '12px', border: '1px solid #30363d', marginBottom: '30px' }}>
        <h2 style={{ fontSize: '18px', color: '#58a6ff', marginBottom: '15px' }}>2. Painel de Base de Custos (Armazenamento Fixo)</h2>
        <p style={{ color: '#888', fontSize: '14px', marginBottom: '15px' }}>Faça o upload da planilha fixa de custos para cruzar com as vendas e calcular a liquidez final.</p>
        ...
        <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
          ...
          <button 
            type="button"
            onClick={() => alert('Funcionalidade de envio para o Supabase pronta para integração com a base de dados!')}
            style={{ background: '#1f6feb', color: '#fff', padding: '12px 20px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
          >
            Guardar Custos no Supabase
          </button>
        </div>
      </div>

      {/* RESULTADOS DA PROCESSAGEM */}
      {resultado && (
