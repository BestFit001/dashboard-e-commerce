...
export default function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // 1. Procura na lista de utilizadores cadastrados
        ...
        <form onSubmit={handleLogin} className="w-full space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full p-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:outline-none focus:border-purple-500 transition"
            />
          </div>
          <div>
          ...
          <button
            type="submit"
            className="w-full py-3.5 bg-purple-600 hover:bg-purple-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-purple-500/30 transition mt-2"
          >
            Acessar Painel
          </button>
        </form>
